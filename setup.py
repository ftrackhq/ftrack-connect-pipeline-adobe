...
class BuildPlugin(setuptools.Command):
    ...
    def run(self):
        ...
        STAGING_PATH = os.path.join(
            BUILD_PATH, 'ftrack-connect-pipeline-adobe-{}'.format(VERSION)
        )
        ...
        result_path = shutil.make_archive(
            os.path.join(
                BUILD_PATH,
                'ftrack-connect-pipeline-adobe-{0}'.format(VERSION),
            ),
            'zip',
            STAGING_PATH,
        )
...
setup(
    ...
    description='Adobe plugin enabling publish, open, load and asset management with ftrack eco system.',
    long_description=open(README_PATH).read(),
    keywords='ftrack',
    url='https://bitbucket.org/ftrack/ftrack-connect-pipeline-adobe',
    author='ftrack',
    ...
    package_dir={'': 'source'},
    use_scm_version={
        'write_to': 'source/ftrack_connect_pipeline_adobe/_version.py',
